...
if (isset($datapost['stu']) && isset($datapost['cma']) && isset($dataget['id']) && isset($dataget['eva'])) {
    ...
    if ($configured) {
        ...
        $urlinstrument = 'assessment_form.php?id=' . $courseid . '&a=' . $cmid . '&t=' . $tool->idtool . '&s=' . $userid .
            '&mode=assess';
        $task = $DB->get_record('block_evalcomix_tasks', ['instanceid' => $cmid]);
        ...
        if (
            $assessmentgrade = $DB->get_record('block_evalcomix_assessments', ['taskid' => $task->id,
        ...
        if ($mode == 'self' || $mode == 'peer') {
            ...
            $condition = false;
            if (
                $evalcomixallowedusers = $DB->get_record('block_evalcomix_allowedusers', ['cmid' => $cmid,
                'assessorid' => $assessorid, 'studentid' => $userid])
            ) {
                $condition = true;
            }
