...
/**
 * Remove EvalCOMIX grades from gradebook
 *
...
if (
    $rst = $DB->get_records(
        ...
        ['userid' => $userid, 'itemid' => $grade->grade_item->id, 'source' => 'evalcomixAdd']
    )
) {
    $sql2 = "SELECT id, finalgrade FROM {grade_grades_history}
                WHERE userid=? AND itemid=? AND source like 'mod/%'
                ORDER BY id DESC";
    $gradeevalcomix2 = $DB->get_records_sql($sql2, [$userid, $grade->grade_item->id]);
    if ($gradeevalcomix2) {// If there is a reestablished value.
        ...
        $gradeevalcomix = $DB->get_records_sql($sql, [$userid, $grade->grade_item->id]);
        if ($gradeevalcomix) {
...
if ($grade->grade_item->itemnumber == 0) {
    ...
    $courseid = $courseid;
    if (
        !$gradeitem = grade_item::fetch(['iteminstance' => $instancia, 'itemmodule' => $modulo, 'courseid' => $courseid,
        'itemnumber' => 0])
    ) {
    ...
    $grade1 = new grade_grade(['itemid' => $gradeitem->id, 'userid' => $student]);
