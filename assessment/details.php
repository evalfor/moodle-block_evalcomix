...
$url = new moodle_url('/blocks/evalcomix/assessment/details.php', ['contextid' => $contextid, 'itemid' => $itemid]);
$PAGE->set_url($url);
...
if (!$task = $DB->get_record('block_evalcomix_tasks', ['id' => $itemid])) {
    throw new \moodle_exception('Wrong parameters');
}

if ($modeei = $DB->get_record('block_evalcomix_modes', ['taskid' => $task->id, 'modality' => 'peer'])) {
    $modeeitime = $DB->get_record('block_evalcomix_modes_time', ['modeid' => $modeei->id]);
    $modesextra = $DB->get_record('block_evalcomix_modes_extra', ['modeid' => $modeei->id]);
} else {
    $modeeitime = '';
}
...
$dataactivity = $DB->get_record($module->name, ['id' => $cm->instance]);
...
if ($task->workteams) {
    if ($groups = groups_get_all_groups($course->id, $user->id)) {
        foreach ($groups as $group) {
            if (
                $coordinator = $DB->get_record('block_evalcomix_coordinators', ['taskid' => $task->id,
                    'groupid' => $group->id])
            ) {
...
if ($assid && has_capability('moodle/block:edit', $context)) {
    $assessdelete = $DB->get_record('block_evalcomix_assessments', ['id' => $assid]);
    if ($assessdelete) {
        $stringmode = 'peer';
        ...
        foreach ($members as $member) {
            ...
            if (
                $assdelete = $DB->get_record('block_evalcomix_assessments', ['taskid' => $task->id,
                    'assessorid' => $assessorid, 'studentid' => $member->id])
            ) {
                block_evalcomix_assessments::delete_assessment(['where' => ['id' => $assdelete->id],
...
$assessments = $DB->get_records('block_evalcomix_assessments', ['studentid' => $user->id, 'taskid' => $itemid]);
...
// Get Weighings.
$weighingteacher = '';
$modality = $DB->get_record('block_evalcomix_modes', ['taskid' => $itemid, 'modality' => 'teacher']);
if ($modality != null) {
...
$weighingself = '';
$inselfperiod = false;
$modality = $DB->get_record('block_evalcomix_modes', ['taskid' => $itemid, 'modality' => 'self']);
if ($modality != null) {
    $gradeobject->self->weighing = $modality->weighing;
    $weighingself = $modality->weighing;
    if ($modeaetime = $DB->get_record('block_evalcomix_modes_time', ['modeid' => $modality->id])) {
...
$inperiod = false;
$weighingpeer = '';
$modality = $DB->get_record('block_evalcomix_modes', ['taskid' => $itemid, 'modality' => 'peer']);
if ($modality != null) {
...
if (!empty($teacherassessments) && $tool = block_evalcomix_get_modality_tool($course->id, $task->id, 'teacher')) {
    foreach ($teacherassessments as $teachergrade) {
        unset($tgrade);
        $tgrade = new stdClass();
        $userassessor = $DB->get_record('user', ['id' => $teachergrade->assessorid]);
...
if (!empty($peerassessments) && $tool = block_evalcomix_get_modality_tool($course->id, $task->id, 'peer')) {
    $now = time();
    if (
        has_capability('moodle/grade:viewhidden', $context) || ($modeeitime &&
        ...
        foreach ($peerassessments as $peergrade) {
            ...
            $isnoextrem = ($grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART ||
                $grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART_WITH_REDUCTION);
            if ($isnoextrem && !$withteachergrade) {
            ...
            } else if ($isnoextrem && $withteachergrade) {
            ...
            if (!$anonymous) {
                $userassessor = $DB->get_record('user', ['id' => $peergrade->assessorid]);
                $name = get_string('gradeof', 'block_evalcomix') . $userassessor->firstname . ' ' . $userassessor->lastname;
        ...
        if (
            ($grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART
                || $grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART_WITH_REDUCTION)
    ...
    ($grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART ||
        $grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART_WITH_REDUCTION) &&
        empty($peerassessments) &&
        is_numeric($gradeobject->peer->weighing) &&
        !$inperiod &&
        $withteachergrade
) {
    $gradeobject->teacher->weighing += $gradeobject->peer->weighing;
...
if ($selfassessment != null && $tool = block_evalcomix_get_modality_tool($course->id, $task->id, 'self')) {
    ...
    } else if ($grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART_WITH_REDUCTION) {
        if (block_evalcomix_grades::is_upper_grade($selfassessment->grade, $mainsetofgrades, $task->threshold)) {
            if (!empty($gradeobject->teacher->grades)) {
                $tgrade->color = BLOCK_EVALCOMIX_GRADE_METHOD_COLOR_UPPER . ' font-weight-bold';
                $gradeobject->self->weighing = 0;
    ...
    if (
        $grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART && is_numeric($gradeobject->self->weighing) && !$inselfperiod
            && $withteachergrade
    ) {
        $gradeobject->teacher->weighing += $gradeobject->self->weighing;
        $gradeobject->self->weighing = 0;
    } else if (
        $grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART_WITH_REDUCTION && is_numeric($gradeobject->self->weighing) &&
            !$inselfperiod && $withteachergrade
    ) {
        $gradeobject->self->weighing = 0;
    }
} else {
...
if ($grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART || $grademethod == BLOCK_EVALCOMIX_GRADE_METHOD_WA_SMART_WITH_REDUCTION) {
