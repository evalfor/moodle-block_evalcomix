...
/**
 * Form to assign student to an assessor
 *
...
$potentialuser = optional_param_array('pu', [], PARAM_RAW);
$allowed = optional_param_array('au', [], PARAM_RAW);
...
if (isset($add) && isset($potentialuser) && is_array($potentialuser)) {
    foreach ($potentialuser as $puser) {
        $params = ['cmid' => $cm->id, 'assessorid' => $assessorid, 'studentid' => $puser];
...
if (isset($remove) && isset($allowed) && is_array($allowed)) {
    foreach ($allowed as $auser) {
        $params = ['cmid' => $cm->id, 'assessorid' => $assessorid, 'studentid' => $auser];
        if ($alloweduser = $DB->get_record('block_evalcomix_allowedusers', $params)) {
            $DB->delete_records('block_evalcomix_allowedusers', ['id' => $alloweduser->id]);
        }
    }
}
...
foreach ($users as $user) {
    $userid = $user->id;
    if ($task->workteams == 1 && $reportevalcomix->same_group($userid, $assessorid)) {
        continue;
    }
