...
/**
 * Forms for assigning assessments among students
 *
...
$PAGE->set_url(new moodle_url('/blocks/evalcomix/assessment/users_form.php', ['id' => $courseid, 'a' => $cm->id]));
...
$task = $DB->get_record('block_evalcomix_tasks', ['instanceid' => $cm->id]);
$coordinators = [];
if ($task) {
    $allcoordinators = $DB->get_records('block_evalcomix_coordinators', ['taskid' => $task->id]);
    foreach ($allcoordinators as $coordinator) {
        $coordinators[$coordinator->userid] = $coordinator;
    }
}

if ($allowedusers = $DB->get_records('block_evalcomix_allowedusers', ['cmid' => $id])) {
...
foreach ($users as $user) {
    ...
    if (isset($coordinators[$user->id])) {
        continue;
    }
    echo '<option ' . $style . ' ' . $selected . ' value="' . $user->id . '">' . $fullname . '</option>';
}
...
echo '
            ...
            <td id="potentialcell">
               <p><label for="addselect" class="font-weight-bold">' . get_string('studentstoassess', 'block_evalcomix') .
               ':</label></p>
