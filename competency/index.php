...
$PAGE->set_url(new moodle_url('/blocks/evalcomix/competency/index.php', ['id' => $courseid]));
...
switch ($option) {
    case 'competency':
        ...
        break;
    case 'type':
        $datas = $DB->get_records('block_evalcomix_comptype', ['courseid' => $courseid]);
        break;
    case 'outcome':
        $datas = $DB->get_records('block_evalcomix_competencies', ['courseid' => $courseid, 'outcome' => 1]);
        break;
}

echo $OUTPUT->header();

echo html_writer::start_tag('div', ['id' => 'change']);
