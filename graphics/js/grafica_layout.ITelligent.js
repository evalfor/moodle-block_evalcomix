...
*/
    ...
    function array_contiene_elto (array, elto) {        
        for (var i = 0; i < array.length; ++i) { if (array[i] == elto) return true; }      
        return false;
    }
    ...
    function mostrar_grafica () { 
		 ...
		 if(divgraficas.style.display == 'none') divgraficas.style.display = 'inline';
    ...
    function cambiar_filtro_alumnos () {
		...
		{		
			...
			if (lstIdAlumnos.length > 0) { mostrar_grafica(); }
				...
				ullimite.style.display = 'none';
				uldispersion.style.display = 'none';
				if(divgraficas.style.display == 'inline') divgraficas.style.display = 'none';
			}
		}
    }
    ...
    function cambio_radiobutton (value) {	   		
		...
		switch (tipoGrafico) {
			case 'graficaperfiltarea': 
				...
				break;
			case 'graficaperfilalumnado':
				if (rbprofesor.checked) intIdTipoRadioButton = 1;							
				else intIdTipoRadioButton = 2;							
				recuperar_y_rellenar_comboalumno_ajax(intIdTarea, idCurso);
				divalumno.style.display = 'inline';
				break;
			case 'graficaperfilatributos': 
				if (rbalumno.checked) intIdTipoRadioButton = 1; 
				else if (rbgrupo.checked) intIdTipoRadioButton = 2; 
				else intIdTipoRadioButton = 3;  
